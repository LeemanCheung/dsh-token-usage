...
export const zh = {
  ...
  trajectoryAnalysisIntro: '选择“分析轨迹”后，仅将事件类型、相对时间、模型路由、状态和 provider Token bucket 发送给上方手动选择的已接入模型；提示词、回复、工具参数与结果始终省略。报告按需运行且不持久化。',
  ...
  analysisPrivacy: '隐私：手动选择的模型只接收白名单元数据和 provider 上报 Token；不发送提示词、回复、工具名称/参数/结果、会话标题/ID 或个人与组织字段。',
  analysis: '轨迹分析',
  analyze: '分析轨迹',
  analyzing: '分析中…',
  loading: '正在读取会话统计…',
...
export const en = {
  ...
  trajectoryAnalysisIntro: 'Analyze trajectory sends only event types, relative timing, model routes, statuses, and provider Token buckets to the manually selected integrated model above. Prompts, replies, tool arguments, and results are always omitted. Reports run on demand and are not persisted.',
  ...
  analysisPrivacy: 'Privacy: the manually selected model receives only allowlisted metadata and provider-reported Token buckets—never prompts, replies, tool names/arguments/results, session titles/IDs, or personal and organization fields.',
