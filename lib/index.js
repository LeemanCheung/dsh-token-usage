...
const tokenUsageRecorderProjectionDefinition = {
	...
	view: (state) => ({
		...
		usage: state.usage,
		models: Object.values(state.models).sort((left, right) => totalTokens$3(right.usage) - totalTokens$3(left.usage) || left.provider.localeCompare(right.provider) || left.model.localeCompare(right.model)),
...
function prepareTrajectory(events) {
	...
	const largestSpan = usageSpans.reduce((largest, span) => largest === void 0 || totalTokens$2(span.usage) > totalTokens$2(largest.usage) ? span : largest, void 0);
	...
	return {
		metrics: {
			...
			tokensPerMinute: minutes > 0 ? Number((totalTokens$2(providerUsage) / minutes).toFixed(2)) : 0,
			activeTokensPerMinute: activeMinutes > 0 ? Number((totalTokens$2(providerUsage) / activeMinutes).toFixed(2)) : 0,
			usage: providerUsage,
			retryUsage,
			spans: usageSpans,
	...
	return {
		...
		pricing: {
			...
			routes: rankedCosts.models.map((model, index) => ({
				route: `route-${index + 1}`,
				...model.totalCostUSD === void 0 ? {} : { estimatedCostUSD: model.totalCostUSD }
			}))
		}
	};
}
...
function usageAnalysisRequest(payload) {
	...
	if (usage === void 0 || compactionUsage === void 0 || !Array.isArray(rawModels) || rawModels.length > 512 || !Array.isArray(rawDays) || rawDays.length > 3660) return;
		...
		const dispose = ctx.connection.rpc.handle(TOKEN_USAGE_RPC_CHANNEL, async (endpoint, payload, signal) => {
			...
			switch (endpoint) {
				...
				case TOKEN_USAGE_RPC_ENDPOINT.analysisModels: {
					...
					if (llm === void 0) return rpcError("Analysis requires an available model service.");
					const models = await analysisModels({
						llm,
						logger: ctx.logger
					});
					...
					return {
						...
						value: {
							models,
							...defaultSelection !== void 0 && isKnownModel(models, defaultSelection) ? { default: {
				...
				case TOKEN_USAGE_RPC_ENDPOINT.usageAnalyze: {
					...
					try {
						const llm = ctx.get("llm");
						if (llm === void 0) return rpcError("Usage analysis requires an available model service.");
						if (!isKnownModel(await analysisModels({
							llm,
							logger: ctx.logger
						}), request.model)) return rpcError("Select one of the currently integrated models before starting analysis.");
						return {
							ok: true,
				...
				case TOKEN_USAGE_RPC_ENDPOINT.trajectoryAnalyze: {
					...
					try {
						const llm = ctx.get("llm");
						if (llm === void 0) return rpcError("Trajectory analysis requires an available model service.");
						if (!isKnownModel(await analysisModels({
							llm,
							logger: ctx.logger
						}), request.model)) return rpcError("Select one of the currently integrated models before starting analysis.");
						...
						return {
							ok: true,
							value: await analyzeTrajectory({ llm }, request.sessionId, events, request.model, request.language, operationSignal)
						};
					} catch (error) {
						if (operationSignal.aborted) throw error;
						return rpcError(error instanceof Error ? error.message : String(error));
