...
function modelEvidence(models) {
	return models.map((model, index) => ({
		model,
		index
	})).sort((left, right) => totalTokens(right.model.usage) - totalTokens(left.model.usage) || right.model.assistantRequests - left.model.assistantRequests || right.model.compactionRequests - left.model.compactionRequests || right.model.usage.uncachedInputTokens - left.model.usage.uncachedInputTokens || right.model.usage.outputTokens - left.model.usage.outputTokens || right.model.usage.cacheReadTokens - left.model.usage.cacheReadTokens || right.model.usage.cacheWriteTokens - left.model.usage.cacheWriteTokens || left.index - right.index).slice(0, MAX_MODEL_ROWS).map(({ model }, index) => ({
		provider: "route",
		model: `route-${index + 1}`,
