...
function adjustUsage(target, value, direction) {
...
function routeOf(type, data) {
	const candidate = type === "request/header" ? isRecord$1(data.header) && isRecord$1(data.header.config) ? data.header.config : void 0 : type === "request/context" ? data : void 0;
	return candidate !== void 0 && typeof candidate.provider === "string" && typeof candidate.model === "string" ? {
...
function prepareTrajectory(events) {
	...
	const rows = [];
	const assistantSamples = /* @__PURE__ */ new Map();
	...
	const toolLatencies = [];
	let previousRoute;
	const assistantKey = (data) => typeof data.turn === "number" && typeof data.step === "number" ? `${data.turn}\u0000${data.step}` : void 0;
	const applyAssistantUsage = (data, rawUsage) => {
		const key = assistantKey(data);
		const next = usageBuckets(rawUsage);
		if (key === void 0 || next === void 0) return false;
		const previous = assistantSamples.get(key);
		if (previous !== void 0) adjustUsage(usage, previous, -1);
	...
	for (const event of events) {
		...
		if (route !== void 0) {
			if (previousRoute !== void 0 && (route.provider !== previousRoute.provider || route.model !== previousRoute.model)) modelSwitches += 1;
			previousRoute = route;
		}
		switch (type) {
			case "turn/start":
			...
			case "step/start": {
				stepCount += 1;
				const key = assistantKey(data);
				if (key !== void 0) openStepKeys.add(key);
				break;
			}
			case "step/end": {
				const key = assistantKey(data);
				if (key !== void 0) openStepKeys.delete(key);
				break;
			}
			case "assistant/message":
				if (!applyAssistantUsage(data, data.usage)) {
					const key = assistantKey(data);
					if (key !== void 0 && !assistantSamples.has(key)) {
						assistantRequests += 1;
						assistantSamples.set(key, {
							uncachedInputTokens: 0,
							outputTokens: 0,
							cacheReadTokens: 0,
							cacheWriteTokens: 0
			...
			case "llm/retry": {
				retries += 1;
				const key = assistantKey(data);
				if (key !== void 0) assistantSamples.delete(key);
				break;
			}
			case "compaction/summary": {
				compactions += 1;
				const compactionUsage = usageBuckets(data.usage);
				if (compactionUsage !== void 0) adjustUsage(usage, compactionUsage, 1);
				break;
			}
			case "approval/asked":
				approvalsAsked += 1;
