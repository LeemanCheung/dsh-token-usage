	...
	for (const event of events) {
		...
		switch (type) {
			...
			case "approval/decided": {
				...
				if (id === void 0 || !approvalRequestIds.has(id) || resolvedApprovalIds.has(id)) orphanApprovalDecisions += 1;
				else resolvedApprovalIds.add(id);
				const outcome = isRecord$1(data.outcome) ? data.outcome.kind : data.outcome;
						...
						return {
							...
							value: await withProgress(request.progressId, async (report) => {
								...
								if (events === void 0) {
									...
									if (persistence === void 0) throw new Error("Trajectory analysis cannot read cold sessions because persistence is unavailable.");
