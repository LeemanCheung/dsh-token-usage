...
const bucketsSchema$1 = z$1.object({
...
const projectionSchema = z$1.object({
	...
	compactionUsage: bucketsSchema$1,
	usage: bucketsSchema$1,
...
const recorderStateSchema = z$1.object({
	...
	days: z$1.record(z$1.string(), bucketsSchema$1),
...
function prepareTrajectory(events, localRoutes) {
...
const integer = z$1.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER);
const identifier = z$1.string().min(1).max(256);
const stamp = z$1.string().datetime({ offset: true }).refine((value) => Number.isFinite(Date.parse(value)));
z$1.string().regex(/^\d{4}-\d{2}-\d{2}$/).refine((value) => {
...
const rate = z$1.number().finite().min(0).max(1e6).nullable();
const ratesSchema = z$1.object({
	uncachedInputTokens: rate,
	outputTokens: rate,
	cacheReadTokens: rate,
	cacheWriteTokens: rate
}).strict();
const periodSchema = z$1.object({
	days: z$1.array(z$1.number().int().min(0).max(6)).min(1).max(7).refine((days) => new Set(days).size === days.length),
	startMinute: z$1.number().int().min(0).max(1439),
	endMinute: z$1.number().int().min(1).max(1440),
	rates: ratesSchema
}).strict().refine((value) => value.startMinute < value.endMinute);
const rateCardSchema = z$1.object({
	id: z$1.string().regex(/^[A-Za-z0-9._-]{1,96}$/),
	label: z$1.string().min(1).max(100),
	provider: identifier,
	model: identifier,
	currency: z$1.enum(["USD", "CNY"]),
	effectiveFrom: stamp,
	effectiveTo: stamp.optional(),
	verifiedAt: stamp,
	source: z$1.enum(["public", "user-defined"]),
	sourceUrl: z$1.string().max(512).url().refine((value) => {
		const url = new URL(value);
		return url.protocol === "https:" && !url.username && !url.password;
	}).optional(),
	rates: ratesSchema,
	timezone: z$1.string().min(1).max(80).refine((value) => {
		try {
	...
	periods: z$1.array(periodSchema).max(32).default([]),
	tiers: z$1.array(z$1.object({
		minimumInput: integer,
		rates: ratesSchema
	}).strict()).max(16).default([]),
	cacheWriteVariants: z$1.object({
		short: rate,
		long: rate
	}).strict().optional()
...
const cardsSchema = z$1.array(rateCardSchema).max(128).superRefine((cards, ctx) => {
...
const moneyBudgetSchema = z$1.object({
	currency: z$1.enum(["USD", "CNY"]),
	amount: z$1.number().finite().positive().max(1e9)
}).strict();
const projectSchema = z$1.object({
	id: identifier,
	name: z$1.string().trim().min(1).max(80),
	tokenBudget: integer,
	moneyBudgets: z$1.array(moneyBudgetSchema).max(2).refine((values) => new Set(values.map((value) => value.currency)).size === values.length).default([])
}).strict();
const assignmentSchema = z$1.object({
	sessionId: identifier,
	projectId: identifier,
	tags: z$1.array(z$1.string().trim().min(1).max(40)).max(12)
}).strict();
const findingSchema = z$1.object({
	ruleId: identifier,
	ruleVersion: z$1.literal("1"),
	severity: z$1.enum([
		"info",
		"warning",
		"error"
	]),
	evidence: z$1.array(z$1.string().max(200)).max(16),
	value: z$1.number().finite(),
	threshold: z$1.number().finite().optional(),
	coverage: z$1.enum([
		"complete",
		"partial",
		"unavailable"
	...
	suggestedAction: z$1.object({
		zh: z$1.string().min(1).max(400),
		en: z$1.string().min(1).max(600)
	}).strict().default({
		zh: "查看证据并确认数据覆盖。",
...
const nodeSchema = z$1.object({
	id: identifier,
	seq: integer,
	kind: z$1.enum(["model", "compaction"]),
	status: z$1.enum([
		"open",
		"completed",
		"retried"
	]),
	finality: z$1.enum(["provisional", "authoritative"]),
	provider: identifier,
	model: identifier,
	usage: bucketsSchema,
	time: stamp.optional()
...
const totalsSchema = z$1.object({
	...
	delta: z$1.object({
		uncachedInputTokens: z$1.number().int().finite(),
		outputTokens: z$1.number().int().finite(),
		cacheReadTokens: z$1.number().int().finite(),
		cacheWriteTokens: z$1.number().int().finite()
	}).strict(),
	requests: integer,
	retries: integer,
	compactions: integer,
...
const snapshotSchema = z$1.object({
	schema: z$1.literal("dsh-token-usage/snapshot-v1"),
	sessionId: identifier,
	generatedAt: stamp,
	revision: identifier,
	firstSeq: integer,
	...
	eventCount: integer,
	totals: totalsSchema,
	reconciliation: z$1.enum(["matched", "mismatch"]),
	findings: z$1.array(findingSchema).max(32),
	nodes: z$1.array(nodeSchema).max(200),
	nodeCount: integer,
	provisionalNodeCount: integer,
	offset: integer,
	nextOffset: integer.nullable(),
	timeCoverage: z$1.enum([
		"complete",
		"partial",
		"unavailable"
	]).default("unavailable"),
	routeCoverage: z$1.enum([
		"complete",
		"partial",
		"unavailable"
	]).default("unavailable"),
	routes: z$1.array(z$1.object({
		provider: identifier,
		model: identifier,
		usage: bucketsSchema
	}).strict()).max(512)
}).strict();
const experimentRunSchema = z$1.object({
	id: identifier,
	experiment: z$1.string().trim().min(1).max(80),
	variant: z$1.enum(["baseline", "candidate"]),
	pair: z$1.string().trim().min(1).max(80),
	task: z$1.string().trim().min(1).max(80),
	size: z$1.string().trim().min(1).max(40),
	conditions: z$1.string().trim().min(1).max(160),
	configLabel: z$1.string().trim().min(1).max(80),
	accepted: z$1.boolean().nullable(),
	generatedAt: stamp,
	revision: identifier,
	usage: bucketsSchema,
	retries: integer,
	...
	costs: z$1.array(z$1.object({
		currency: z$1.enum(["USD", "CNY"]),
		amount: z$1.number().finite().nonnegative().max(Number.MAX_SAFE_INTEGER),
		complete: z$1.boolean(),
		basis: z$1.string().max(200),
		fingerprint: z$1.string().max(128).optional()
	}).strict()).max(2)
}).strict();
const configurationSchema = z$1.object({
	cards: cardsSchema,
	projects: z$1.array(projectSchema).max(64),
	assignments: z$1.array(assignmentSchema).max(5e3),
	experiments: z$1.array(experimentRunSchema).max(500),
	thresholds: z$1.object({
		retryShare: z$1.number().min(0).max(1),
		compactionShare: z$1.number().min(0).max(1)
	}).strict(),
	moneyBudgets: z$1.array(moneyBudgetSchema).max(2).refine((values) => new Set(values.map((value) => value.currency)).size === values.length),
	shareSummary: z$1.boolean()
...
const ledgerEntrySchema = z$1.object({
	requestKey: z$1.string().regex(/^[a-f0-9]{64}$/).optional(),
	id: identifier,
	routeId: z$1.string().regex(/^[a-f0-9]{32}$/),
	kind: z$1.enum(["usage-analysis", "trajectory-analysis"]),
	startedAt: stamp,
...
const priceRevisionSchema = z$1.object({
	revision: integer,
	at: stamp,
	reason: z$1.enum([
		"initial",
	...
	digest: z$1.string().regex(/^[a-f0-9]{64}$/),
	cards: cardsSchema
}).strict();
const requestKeySchema = z$1.object({
	key: z$1.string().regex(/^[a-f0-9]{64}$/),
	fingerprint: z$1.string().regex(/^[a-f0-9]{64}$/),
	at: stamp
}).strict();
const stateSchema = z$1.object({
	schema: z$1.literal(SCHEMA),
	revision: integer,
	config: configurationSchema,
	ledger: z$1.array(ledgerEntrySchema).max(512),
	evictedEntries: integer,
	priceRevision: integer.default(0),
	priceHistory: z$1.array(priceRevisionSchema).max(16).default([]),
	evictedPriceRevisions: integer.default(0),
	requestKeys: z$1.array(requestKeySchema).max(2048).default([]),
...
function migrateState(value) {
	if (typeof value === "object" && value !== null && "schema" in value && value.schema === "dsh-token-usage/workbench-v1") return boundedParse(stateSchema, {
		...value,
		schema: SCHEMA
	});
...
const snapshotRequestSchema = z$1.object({
	sessionId: identifier,
	offset: integer.max(2e5).default(0),
	revision: identifier.optional()
}).strict();
const configRequestSchema = z$1.object({
	revision: integer,
	config: configurationSchema
}).strict();
//#endregion
...
function withNodeEvidence(findings, nodes) {
	return findings.map((item) => ({
		...item,
		nodes: nodes.flatMap((node, index) => {
			return (item.ruleId === "retry-share" ? node.status === "retried" : item.ruleId === "compaction-share" ? node.kind === "compaction" : item.ruleId === "route-coverage" ? node.provider === "unknown" || node.model === "unknown" : item.ruleId === "time-coverage" ? !node.time : false) ? [{
				id: node.id,
				seq: node.seq,
				index
			}] : [];
...
function diagnose(totals, thresholds) {
	const findings = [];
	const push = (ruleId, severity, value, evidence, threshold, coverage = "complete") => findings.push(finding(ruleId, severity, value, evidence, coverage, "session", threshold));
	const tokens = total(totals.usage);
	if (bucketKeys.some((key) => totals.delta[key] !== 0)) push("reconciliation", "error", bucketKeys.reduce((sum, key) => sum + Math.abs(totals.delta[key]), 0), bucketKeys.map((key) => `${key}: ${totals.delta[key]}`), void 0, "partial");
	if (tokens && total(totals.retry) / tokens >= thresholds.retryShare && total(totals.retry)) push("retry-share", "warning", total(totals.retry) / tokens, [
		`retryTokens: ${total(totals.retry)}`,
		`allTokens: ${tokens}`,
		`retries: ${totals.retries}`
	], thresholds.retryShare);
	...
	if (totals.orphanTools) push("orphan-tools", "warning", totals.orphanTools, [`unpairedToolEvents: ${totals.orphanTools}`], void 0, "partial");
	if (totals.openTurns || totals.openSteps) push("open-lifecycle", "info", totals.openTurns + totals.openSteps, [`openTurns: ${totals.openTurns}`, `openSteps: ${totals.openSteps}`], void 0, "partial");
	if (totals.unresolvedApprovals) push("unresolved-approvals", "warning", totals.unresolvedApprovals, [`unresolvedApprovals: ${totals.unresolvedApprovals}`], void 0, "partial");
	if (!tokens) push("usage-unavailable", "info", 0, ["No provider-reported Token usage"], void 0, "unavailable");
	return findings;
}
//#endregion
//#region src/workbench/snapshot.ts
/** Parallel work is measured by the union of active intervals. */
function intervalUnion(intervals) {
	const sorted = intervals.filter(([start, end]) => Number.isFinite(start) && Number.isFinite(end) && end >= start).map((value) => [...value]).sort((a, b) => a[0] - b[0]);
	let duration = 0, start, end = 0;
	...
	return duration + (start === void 0 ? 0 : end - start);
}
function buildSnapshot(sessionId, events, thresholds, now = Date.now()) {
	if (events.length === 0) throw new Error("Session has no events");
	if (events.length > 2e5) throw new Error("Session exceeds the bounded 200,000-event inspection limit");
	...
	const nodes = metrics.spans.map((span) => ({
		...
		kind: span.kind,
		status: span.status,
		finality: span.finality,
		...routes.get(span.model) ?? {
			provider: "unknown",
			model: "unknown"
		},
		usage: { ...span.usage },
		...Number.isFinite(times.get(span.seq)) && Math.abs(times.get(span.seq)) <= 864e13 ? { time: new Date(times.get(span.seq)).toISOString() } : {}
	}));
	const usageByKind = (kind) => nodes.filter((node) => kind === "compaction" ? node.kind === "compaction" : node.kind === "model" && (kind === "retry" ? node.status === "retried" : node.status !== "retried")).reduce((sum, node) => add(sum, node.usage), zero());
	const grouped = /* @__PURE__ */ new Map();
	for (const node of nodes) {
		const key = JSON.stringify([node.provider, node.model]), entry = grouped.get(key) ?? {
			provider: node.provider,
			model: node.model,
			usage: zero()
		};
		entry.usage = add(entry.usage, node.usage);
		grouped.set(key, entry);
	}
	if (grouped.size > 512) throw new Error("Session has too many routes to inspect safely");
	const open = /* @__PURE__ */ new Map(), intervals = [];
	for (const event of events) {
		...
		if (String(event.type) === "turn/end" && open.has(data.turn)) {
			intervals.push([open.get(data.turn), event.time]);
			open.delete(data.turn);
		}
	}
	for (const time of open.values()) intervals.push([time, events.at(-1).time]);
	const totals = {
		usage: { ...metrics.usage },
		retry: usageByKind("retry"),
		compaction: usageByKind("compaction"),
		ordinary: usageByKind("ordinary"),
		attributed: { ...metrics.reconciliation.attributedUsage },
		delta: { ...metrics.reconciliation.delta },
		requests: metrics.assistantRequests,
		retries: metrics.retries,
		compactions: metrics.compactions,
		toolCalls: metrics.toolCalls,
		toolErrors: metrics.toolErrors,
		orphanTools: metrics.orphanToolCalls + metrics.orphanToolResults,
		openTurns: metrics.openTurns,
		openSteps: metrics.openSteps,
		unresolvedApprovals: metrics.unresolvedApprovals,
		durationMs: metrics.durationMs,
		activeDurationMs: intervalUnion(intervals),
	...
	const base = {
		schema: "dsh-token-usage/snapshot-v1",
		sessionId,
		...
		eventCount: events.length,
		totals,
		reconciliation: bucketKeys.every((key) => totals.delta[key] === 0) ? "matched" : "mismatch",
		timeCoverage,
		routeCoverage,
		findings: withNodeEvidence(findings, nodes),
		nodeCount: nodes.length,
		provisionalNodeCount: nodes.filter((node) => node.finality !== "authoritative").length,
...
function priceDigest(cards) {
	return createHash("sha256").update(stableJson([...cards].sort((a, b) => a.id.localeCompare(b.id)))).digest("hex");
}
function ensurePriceHistory(state, now = Date.now()) {
	if (!state.priceHistory.length) state.priceHistory.push({
		revision: state.priceRevision,
		at: new Date(now).toISOString(),
		reason: "initial",
		digest: priceDigest(state.config.cards),
		cards: structuredClone(state.config.cards)
	});
}
function revisePrices(state, cards, reason, now = Date.now()) {
	...
	state.priceHistory.push({
		...
		at: new Date(now).toISOString(),
		reason,
		digest: priceDigest(parsed),
		cards: structuredClone(parsed)
	});
	state.config.cards = parsed;
	while (state.priceHistory.length > 16 || state.priceHistory.length > 1 && JSON.stringify(state.priceHistory).length > 15e5) {
...
const storageSchema = z.object({ data: z.string().max(MAX_STATE_CHARS).default("") });
const receiptRequest = z$1.object({
	sessionId: identifier,
	revision: identifier,
	currency: z$1.enum(["USD", "CNY"]),
	mode: z$1.enum(["historical-reference", "revaluation"]),
	at: stamp,
	priceRevision: z$1.number().int().nonnegative().optional()
}).strict();
const rollbackRequest = z$1.object({
	revision: z$1.number().int().nonnegative(),
	targetPriceRevision: z$1.number().int().nonnegative(),
	confirm: z$1.literal("restore-price-revision")
}).strict();
const emptyRequest = z$1.object({}).strict();
const clearRequest = z$1.object({ confirm: z$1.literal("clear-analysis-ledger") }).strict();
...
function createWorkbenchHost(ctx) {
	...
	try {
		...
		state.ledger = state.ledger.map((entry) => entry.status === "running" ? {
			...entry,
			status: "interrupted",
	...
	function cached(sessionId, revision) {
		const key = JSON.stringify([sessionId, revision]), value = archives.get(key);
		if (!value || Date.now() - value.touched > 6e5) {
			archives.delete(key);
			throw new Error("Snapshot expired; refresh the local inspection");
		}
		value.touched = Date.now();
		return value.archive;
	}
	function remember(archive) {
		archives.set(JSON.stringify([archive.base.sessionId, archive.base.revision]), {
			archive,
			touched: Date.now()
		});
	...
	async function track(service, kind, route, signal, run, identity) {
		signal.throwIfAborted();
		const entry = {
			id: randomUUID(),
			routeId: createHash("sha256").update(JSON.stringify([route.provider, route.model])).digest("hex").slice(0, 32),
			kind,
		...
		if (identity) {
			z$1.string().min(1).max(128).parse(identity.requestId);
			...
			await mutate((draft) => {
				signal.throwIfAborted();
				const retained = draft.requestKeys.filter((item) => Date.now() - Date.parse(item.at) < 30 * 864e5);
				draft.evictedRequestKeys += draft.requestKeys.length - retained.length;
				draft.requestKeys = retained;
				const prior = retained.find((item) => item.key === key) ?? (draft.ledger.some((item) => item.requestKey === key) ? { fingerprint } : void 0);
				...
				draft.requestKeys.push({
					key,
					fingerprint,
					at: entry.startedAt
				});
				while (draft.requestKeys.length > 2048) {
					draft.requestKeys.shift();
					draft.evictedRequestKeys++;
				}
				draft.ledgerStartedAt ??= entry.startedAt;
			});
		...
		const unfinished = /* @__PURE__ */ new Set();
		await checkpoint(entry);
		const observed = new Proxy(service, { get(target, property, receiver) {
				...
				return {
					...
					stream: async function* (...streamArgs) {
						...
						for await (const chunk of prepared.stream(...streamArgs)) {
							if (chunk.type === "usage") {
								const value = bucketsSchema.safeParse({
									uncachedInputTokens: chunk.usage.inputTokens,
									outputTokens: chunk.usage.outputTokens,
									cacheReadTokens: chunk.usage.cacheReadTokens ?? 0,
									cacheWriteTokens: chunk.usage.cacheWriteTokens ?? 0
								});
								if (value.success) {
									streamUsage.set(streamId, value.data);
									entry.usage = [...streamUsage.values()].reduce((sum, usage) => add(sum, usage), zero());
									entry.finality = "provisional";
									if (Date.now() - lastCheckpoint >= 1e3) {
	...
	async function handle(endpoint, payload, signal) {
		try {
			...
			switch (endpoint) {
				...
				case "workbench/price-rollback": {
					...
					await mutate((draft) => {
						...
						revisePrices(draft, prior.cards, "rollback");
						draft.revision++;
					});
					return {
						ok: true,
				...
				case "workbench/ledger-clear":
					...
					await mutate((draft) => {
						...
						draft.ledger = [];
						draft.evictedEntries = 0;
						draft.ledgerClearedAt = (/* @__PURE__ */ new Date()).toISOString();
					});
					return {
						ok: true,
						value: structuredClone(state)
				...
				case "workbench/snapshot": {
					...
					if (request.revision) return {
						ok: true,
						value: pageSnapshot(cached(request.sessionId, request.revision), request.offset)
					};
				...
				case "workbench/receipt-cost": {
					...
					const priceVersion = request.priceRevision === void 0 ? state.priceRevision : request.priceRevision;
					...
					const estimates = archive.nodes.map((node) => quote(request.mode === "historical-reference" && !node.time ? [] : book.cards, node, node.usage, {
						...
						mode: request.mode,
						at: request.mode === "historical-reference" ? node.time ?? request.at : request.at,
						timingKnown: request.mode === "revaluation",
						requestInputKnown: node.finality === "authoritative"
					}));
					let estimate = sumQuotes(estimates, request.currency);
					if (archive.base.reconciliation !== "matched") estimate = {
						...estimate,
						totalTokens: total(archive.base.totals.usage),
						amount: null,
						upper: null,
						status: "partial",
						unavailable: [...estimate.unavailable, "token-reconciliation-mismatch"]
					};
					const known = estimates.map((estimate, index) => ({
						id: archive.nodes[index].id,
						amount: estimate.amount
					})).filter((item) => item.amount !== null).sort((a, b) => b.amount - a.amount);
					return {
						ok: true,
						value: {
							estimate,
							largestPricedNode: known[0] ?? null,
							revision: archive.base.revision,
							priceRevision: priceVersion,
							priceDigest: priceDigest(book.cards)
						}
					};
				}
			...
			return failure(error instanceof z$1.ZodError ? "Invalid workbench payload" : error instanceof Error ? error.message : "Workbench request failed");
		}
	}
	return {
		handle,
			...
			switch (endpoint) {
				...
				case TOKEN_USAGE_RPC_ENDPOINT.usageAnalyze: {
					...
					try {
						return {
							ok: true,
							value: await withProgress(request.progressId, (report) => getWorkbench().track(runtime.llm, "usage-analysis", request.model, analysisSignal, (llm) => analyzeTokenUsage({ llm }, request.input, request.model, request.language, analysisSignal, report), request.progressId ? {
								requestId: request.progressId,
								fingerprint: JSON.stringify([request.input, request.language])
							} : void 0))
						};
					} catch (error) {
					...
					try {
						return {
							...
							value: await withProgress(request.progressId, async (report) => {
								...
								return getWorkbench().track(runtime.llm, "trajectory-analysis", request.model, analysisSignal, (llm) => analyzeTrajectory({ llm }, request.sessionId, events, request.model, request.language, analysisSignal, report), request.progressId ? {
									requestId: request.progressId,
									fingerprint: JSON.stringify([request.sessionId, request.language])
								} : void 0);
							})
						};
					} catch (error) {
