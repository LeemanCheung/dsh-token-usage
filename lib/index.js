...
const SCHEMA = "dsh-token-usage/workbench-v1";
...
function diagnose(totals, thresholds) {
	const findings = [];
	const push = (ruleId, severity, value, evidence, threshold, coverage = "complete") => findings.push({
		ruleId,
		ruleVersion: "1",
		severity,
		value,
...
function buildSnapshot(sessionId, events, thresholds, now = Date.now()) {
	...
	const base = {
		...
		findings: diagnose(totals, thresholds),
...
function createWorkbenchHost(ctx) {
	...
	async function track(service, kind, route, signal, run) {
		const entry = {
			id: randomUUID(),
			...
			switch (endpoint) {
				...
				case TOKEN_USAGE_RPC_ENDPOINT.usageAnalyze: {
					...
					try {
						return {
							ok: true,
							value: await withProgress(request.progressId, (report) => getWorkbench().track(runtime.llm, "usage-analysis", request.model, analysisSignal, (llm) => analyzeTokenUsage({ llm }, request.input, request.model, request.language, analysisSignal, report)))
						};
					} catch (error) {
					...
					try {
						return {
							...
							value: await withProgress(request.progressId, async (report) => {
								...
								return getWorkbench().track(runtime.llm, "trajectory-analysis", request.model, analysisSignal, (llm) => analyzeTrajectory({ llm }, request.sessionId, events, request.model, request.language, analysisSignal, report));
							})
						};
					} catch (error) {
						if (analysisSignal.aborted) throw error;
