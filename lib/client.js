	...
	factory: (require) => {
		...
		*/
		...
		function safeModelMarkdown(markdown) {
			return markdown.replace(/!\[([^\]]*)\]\(([^)\n]+)\)/g, (_match, alt, destination) => `[${alt.length === 0 ? "image" : alt}](${destination})`).replace(/!\[([^\]]*)\]\[([^\]]*)\]/g, (_match, alt, reference) => `[${alt.length === 0 ? "image" : alt}][${reference}]`).replace(/!\[([^\]]*)\]/g, (_match, alt) => alt.length === 0 ? "image" : alt).replace(/<(?=[A-Za-z/!?])/g, "&lt;");
		}
		//#endregion
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(AnalysisSummaryGroup, {
								...
								rows: [
									...
									{
										label: t("analysisApprovalPersistent"),
										value: formatTokens(metrics.approvalsAllowedAlways)
									},
									{
		...
		function TokenUsageSection({ close, useSessions, useBudget, setBudget, download, saveTrajectoryAnalysis, openSession, listAnalysisModels, analyzeTokenUsage, analyzeTrajectory, t }) {
