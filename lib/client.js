window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TokenUsageSection({ useSessions, useBudget, setBudget, download, analyzeTrajectory, t }) {
			...
			const runAnalysis = (row) => {
				...
				analyzeTrajectory(row.id, controller.signal).then((value) => {
					if (analysisController.current === controller && !controller.signal.aborted) setAnalysis({
						status: "ready",
						title: row.title,
						value
					});
		...
		function trajectoryAnalysisOf(value) {
			if (!isRecord(value) || value.schema !== "dsh-token-usage/trajectory-analysis-v1" || typeof value.sessionId !== "string" || typeof value.generatedAt !== "string" || typeof value.truncated !== "boolean" || typeof value.report !== "string" || !isRecord(value.model) || typeof value.model.provider !== "string" || typeof value.model.model !== "string") return void 0;
		...
		async function requestTrajectoryAnalysis(connection, sessionId, language, signal) {
		...
		const zh = {
			...
			trajectoryAnalysisIntro: "在会话记录中选择“分析轨迹”，使用 DSH 当前配置模型审查调用链、合规性、异常、速率、性能、可靠性和 Token 效率。分析按需运行，不持久化报告。",
			...
			analysisPrivacy: "隐私提示：分析会将经过长度限制和常见凭据脱敏的会话轨迹发送给当前配置的模型提供方。",
			analysis: "轨迹分析",
			analyze: "分析轨迹",
			analyzing: "分析中…",
			loading: "正在读取会话统计…"
		...
		const en = {
			...
			trajectoryAnalysisIntro: "Select Analyze trajectory on a session to use the currently configured DSH model for call-chain, compliance, anomaly, rate, performance, reliability, and token-efficiency review. Reports run on demand and are not persisted.",
			...
			analysisPrivacy: "Privacy: analysis sends a length-bounded trajectory with common credential patterns redacted to the currently configured model provider.",
		...
		function apply(ctx) {
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				inject: () => ({
					...
					analyzeTrajectory: (sessionId, signal) => requestTrajectoryAnalysis(connection, sessionId, ctx.locale.getLocale().active, signal)
				})
			}, TokenUsageSection));
		}
		//#endregion
