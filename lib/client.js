window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function periodInsight(records, days, now = Date.now()) {
			...
			})).filter((record) => totalTokens$3(record.usage) > 0);
			const peak = active.reduce((highest, record) => highest === void 0 || totalTokens$3(record.usage) > totalTokens$3(highest.usage) ? record : highest, void 0);
			return {
				days,
		...
		function runRateInsight(records, now = Date.now()) {
			...
			const averageDailyTokens = dates.reduce((sum, date) => sum + totalTokens$3(byDate.get(date) ?? zeroBuckets$1()), 0) / dates.length;
			return {
		...
		function dailyAnomalyInsight(records, now = Date.now()) {
			...
			const tokens = totalTokens$3(byDate.get(date) ?? zeroBuckets$1());
			const activeBaseline = datesEndingOn(now - 2 * 864e5, 28).map((baselineDate) => totalTokens$3(byDate.get(baselineDate) ?? zeroBuckets$1())).filter((value) => value > 0);
		...
		function dailyContributors(sessions, date) {
			return sessions.flatMap((session) => {
				...
				if (record === void 0 || totalTokens$3(record.usage) === 0) return [];
				return [{
					id: session.id,
					title: session.title,
					usage: { ...record.usage }
				}];
			}).sort((left, right) => totalTokens$3(right.usage) - totalTokens$3(left.usage) || left.title.localeCompare(right.title) || left.id.localeCompare(right.id));
		}
		//#endregion
		...
		function usageEfficiencyInsight(usage, compactionUsage, models, assistantAttempts, compactionAttempts) {
			...
			const unattributedTokens = models.filter(isUnattributed$1).reduce((sum, model) => sum + totalTokens$2(model.usage), 0);
			const assistantTokens = Math.max(0, total - totalTokens$2(compactionUsage));
			const topRoutes = attributed.map((model) => ({
				provider: model.provider,
				model: model.model,
				tokens: totalTokens$2(model.usage)
			...
			return {
				...
				compactionTokenShare: total === 0 ? void 0 : totalTokens$2(compactionUsage) / total,
		...
		*/
		...
		var TokenUsageSection_module_css_default = {
			...
			"sessionLink": "_3Hb-wa_sessionLink",
			...
			"modelSort": "_3Hb-wa_modelSort",
			"pricingNotice": "_3Hb-wa_pricingNotice",
			"priceValue": "_3Hb-wa_priceValue",
			...
			"priceUnknown": "_3Hb-wa_priceUnknown",
			...
			"anomalyNotice": "_3Hb-wa_anomalyNotice",
		...
		function sortedModelHotspots(models, sort) {
			return models.slice().sort((left, right) => modelSortValue(right, sort) - modelSortValue(left, sort) || totalTokens(right.usage) - totalTokens(left.usage) || left.provider.localeCompare(right.provider) || left.model.localeCompare(right.model));
		}
		...
		function sessionRow(summary) {
			...
			if (usage === void 0 || totalTokens(usage) === 0 && assistantRequests === 0 && compactionRequests === 0) return null;
			return {
				id: summary.id,
