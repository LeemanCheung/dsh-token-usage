...
成本是**静态公开费率估算，不是 provider 账单**。内置表以 USD / 1M Token 计价，当前覆盖 OpenAI 第一方 API 的精确 `provider: 'openai'` 路由：`gpt-5`、`gpt-5-mini`、`gpt-5-nano`、`gpt-4.1`、`gpt-4.1-mini`、`gpt-4.1-nano`、`gpt-4o` 和 `gpt-4o-mini`（及列出的 API 版本别名）。费率依据 [OpenAI 官方 API Pricing](https://developers.openai.com/api/docs/pricing)，表内最近基准日为 **2025-08-07**，不会联网实时刷新。
...
| 优化建议 | 3–7 条带 P0/P1/P2、证据、预期 Token/成本效率收益、置信度和实施工作量的建议。 |
...
- AI 用量分析只可选择当前能由已接入 provider 列出的模型；每日趋势证据最多传递最新 366 天。内置 USD 费率表不是实时账单或汇率服务，仅覆盖文档列出的精确 OpenAI 路由；模型建议不替代价格、延迟或质量观测。
