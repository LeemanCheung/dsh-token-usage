...
| **确定性证据** | 用量报告只依据聚合 bucket、路由、次数和 UTC 日期；轨迹报告同时获得墙钟/活跃 Token 速率、工具配对与延迟、未闭合 Turn/Step、模型切换、重试、压缩、审批与子代理等本地计算指标。 |
...
| 速率与性能 | 墙钟事件/分钟、墙钟与活跃 Token/分钟、工具平均/最大延迟、调用突发和等待热点。 |
| Token 与上下文效率 | 输入/输出/缓存分布、压缩压力、上下文增长、模型切换与潜在浪费。 |
| 工具与子代理成效 | 工具成功率、错误聚类、委派收益、重复工作与汇总质量。 |
| 可靠性与生命周期 | Turn/Step 未闭合数量、Tool call/result 双向孤立数量、模型切换、完成原因和中断操作。 |
